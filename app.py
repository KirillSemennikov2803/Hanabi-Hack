...
def hello_world():
    ...
    d = {}
    file = io.open("lol.py", "r", encoding="UTF-8")
    ...
    for line in file:
        ...
        if (Varfrom == -1 and Varimport != -1 and VarAs == -1):
            ...
            for i in range(0, countofDf):
                if (line.find(df.loc[i, "var"]) != -1 and not (line[line.find(df.loc[i, "var"]) - 1] >= "A" and line[
                    line.find(df.loc[i, "var"]) - 1] <= "z") and not (
                        line[line.find(df.loc[i, "var"]) + len(df.loc[i, "var"])] >= "A" and line[
                    line.find(df.loc[i, "var"]) + len(df.loc[i, "var"])] <= "z") and not (
                        line[line.find(df.loc[i, "var"]) + len(df.loc[i, "var"])] >= "0" and line[
                    line.find(df.loc[i, "var"]) + len(df.loc[i, "var"])] <= "9")):
                    if (line.find("=") != -1 and line[line.find("=") + 1] != "=" and line.find(
                            df.loc[i, "var"]) > line.find("=")):
